...
#
#     > powershell -nop -c "iex(New-Object Net.WebClient).DownloadString('https://git.io/JJVlQ')"
#
...
$ui = @(
	...
	"Stop-EdgePDF",
	"EnableSecondInTaskBar"			#"HideSecondInTaskBar"
)
...
Function InstallTitusProgs {
	...
	Start-BitsTransfer -Source "https://raw.githubusercontent.com/PWalll/win10-init/master/ooshutup10.cfg" -Destination ooshutup10.cfg
...
Function InstallPrograms {
	$programs = @(
		...
		@("ifranview", "IfranView"),
		@("F.lux","F.lux"),
		@("mpc-hc", "Media Player Classic (VLC Alternative)")
	)

	$programs | ForEach-Object -Process {
		$app = $_
		$name = $app[1]
		Write-Output "Installing $name"
		choco install $app[0] -y
...
# Hide seconds on taskbar clock
Function DisableSecondInTaskBar {
	Write-Output "Adding seconds to taskbar."
...
Function HideEditPaint3D {
	...
	$extensions | ForEach-Object -Process {
		New-ItemProperty -Path "Registry::HKEY_CLASSES_ROOT\SystemFileAssociations\$_\Shell\3D Edit" -Name ProgrammaticAccessOnly -PropertyType String -Value "" -Force
	}
}
...
Function DebloatAll {
    ...
    $Bloatware | ForEach-Object -Process {
		$Bloat = $_
